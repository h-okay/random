...
# Model import
MODEL = joblib.load(os.path.join(PARENT_PATH, "resources", "data_preserved_model.pkl"))


@st.cache
def get_data():
    return pd.read_csv(os.path.join(PARENT_PATH, "resources", "diamonds.csv"))
...
st.header("Diamonds 💎")
st.write(
    """Use filter below to filter the data. You can use side menu to get
an estimated value."""
)
...
clarity_select = st.multiselect("Clarity", clarity_options, default=clarity_options)

after_filter = df[
    (df["cut"].isin(cut_select))
    & (df["color"].isin(color_select))
    & (df["clarity"].isin(clarity_select))
]

# Show dataframe
st.dataframe(after_filter)
st.write(
    str(len(after_filter)) + " rows, " + str(len(after_filter.columns)) + " columns"
)
...
cut_opt = st.sidebar.select_slider(
    "Cut", options=["Fair", "Good", "Very Good", "Premium", "Ideal"], value="Very Good"
)
color_opt = st.sidebar.select_slider(
    "Color", options=["J", "I", "H", "G", "F", "E", "D"], value="G"
)
clarity_opt = st.sidebar.select_slider(
    "Clarity", options=["I1", "SI2", "VS2", "VS1", "VVS2", "VVS1", "IF"], value="VS1"
)
